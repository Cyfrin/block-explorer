...
export default function useGoToProduction(context = useContext()) {
  const walletContext = {
  ...
  const goToProduction = async (contractAddress: string) => {
    ...
    try {
      ...
      try {
        processException(e, "Failed to go to production");
  ...
  return {
    ...
    // Go to production state
    isProcessing,
    error,
    txHash,

    // Go to production actions
    goToProduction,
    reset,
  };
}
