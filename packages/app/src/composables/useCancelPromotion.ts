...
export default function useCancelPromotion(context = useContext()) {
  const walletContext = {
  ...
  const cancelPromotion = async (agreementAddress: string) => {
    ...
    try {
      ...
      const tx = await contract.cancelPromotion(agreementAddress, {
      ...
      try {
        processException(e, "Failed to cancel promotion");
  ...
  return {
    ...
    // Cancel promotion state
    isProcessing,
    error,
    txHash,

    // Cancel promotion actions
    cancelPromotion,
    reset,
  };
}
